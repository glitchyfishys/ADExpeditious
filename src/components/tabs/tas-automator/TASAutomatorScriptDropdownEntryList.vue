<script>
export default {
  name: "TASAutomatorScriptDropdownEntryList",
  data() {
    return {
      currentScriptID: 0,
      runningScriptID: 0,
  ...
  computed: {
    scripts() {
      return Object.values(player.speedrun.TASAutomator.scripts).map(script => ({
        id: script.id,
        name: script.name,
      }));
    },
    ...
    scriptCount() {
      return Object.keys(player.speedrun.TASAutomator.scripts).length;
    },
    maxScriptCount() {
  ...
  methods: {
    update() {
      ...
      this.isPaused = TASAutomatorBackend.isOn && !TASAutomatorBackend.isRunning;
      this.canMakeNewScript = this.scriptCount < this.maxScriptCount;
    },
    changeScriptID(newID) {
      this.currentScriptID = newID;
      player.speedrun.TASAutomator.state.editorScript = newID;
      this.updateCurrentScriptID();
    },
    createNewScript() {
      const newScript = TASAutomatorBackend.newScript();
    ...
    updateCurrentScriptID() {
      ...
      TASAutomatorBackend.stack.clear();
      TASAutomatorBackend.state.topLevelScript = this.currentScriptID;
    },
