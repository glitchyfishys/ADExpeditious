...
export default {
  ...
  methods: {
    update() {
      ...
      this.unsortedEvents = TASAutomatorData.eventLog;
    ...
    clearLog() {
      TASAutomatorData.clearEventLog();
      this.update();
    },
    sortStyle(selected) {
      return {
    ...
    timestamp(entry) {
      switch (this.timestampMode) {
        ...
        case AUTOMATOR_EVENT_TIMESTAMP_MODE.THIS_REALITY:
          return `, ${TimeSpan.fromSeconds(entry.thisReality).toStringShort()}`;
    ...
    scrollToLine(line) {
      ...
      TASAutomatorHighlighter.updateHighlightedLine(line, LineEnum.Event);
    }
  }
};
