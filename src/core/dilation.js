...
export function buyDilationUpgrade(id, bulk = 1) {
  ...
  if (upgrade.config.pelleOnly && !PelleRifts.paradox.milestones[0].isEffectActive) return false;
...
export function getDilationGainPerSecond() {
  ...
  dtRate = dtRate.times(
    Math.clampMin(Decimal.log10(Replicanti.amount) * getAdjustedGlyphEffect("replicationdtgain"), 1)).mul(player.speedrun.mods.DTMul);
  if (Enslaved.isRunning && !dtRate.eq(0)) dtRate = Decimal.pow10(Math.pow(dtRate.plus(1).log10(), 0.85) - 1);
  if (V.isRunning) dtRate = dtRate.pow(0.5);
  return dtRate.pow(player.speedrun.mods.DTPow);
}
...
export function tachyonGainMultiplier() {
  ...
  return DC.D1.timesEffectsOf(
    ...
    RealityUpgrade(8),
    RealityUpgrade(15)
  ).mul(player.speedrun.mods.TPMul).pow(pow).pow(player.speedrun.mods.TPPow);
}
...
export function dilatedValueOf(value) {
  ...
  const dilationPenalty = 0.77 * Effects.product(DilationUpgrade.dilationPenalty);
...
class DilationUpgradeState extends SetPurchasableMechanicState {
  ...
  purchase() {
    if (this.config.pelleOnly && !PelleRifts.paradox.milestones[0].isEffectActive) return false;
